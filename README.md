...
```bash
curl http://localhost:8080/api/no-cache/products/1
curl http://localhost:8080/api/no-cache/products/1
curl http://localhost:8080/api/no-cache/products/1
```
...
- 매번 약 300ms 지연이 발생합니다.
- 로그에 DB query occurred 메시지가 매번 출력됩니다.

## 실습 2. Local Cache 확인

```bash
curl http://localhost:8080/api/local-cache/products/1
curl http://localhost:8080/api/local-cache/products/1
curl http://localhost:8080/api/local-cache/products/1
...
```bash
curl http://localhost:8080/api/ttl-cache/products/1
curl http://localhost:8080/api/ttl-cache/products/1
sleep 11
curl http://localhost:8080/api/ttl-cache/products/1
curl http://localhost:8080/api/cache/stats
...
```bash
curl http://localhost:8080/api/size-limit-cache/products/1
curl http://localhost:8080/api/size-limit-cache/products/2
curl http://localhost:8080/api/size-limit-cache/products/3
curl http://localhost:8080/api/size-limit-cache/products/4
...
- `sizeLimitProducts`의 maximumSize는 3입니다.
- 4개 이상의 Key를 조회하면 eviction이 발생할 수 있습니다.

## 실습 5. 잘못된 Cache Key 확인

```bash
curl "http://localhost:8080/api/bad-key/products/search?category=BOOK&sort=PRICE"
curl "http://localhost:8080/api/bad-key/products/search?category=BOOK&sort=LATEST"
```
...
## 실습 6. 올바른 Cache Key 확인

```bash
curl "http://localhost:8080/api/good-key/products/search?category=BOOK&sort=PRICE"
curl "http://localhost:8080/api/good-key/products/search?category=BOOK&sort=LATEST"
```
